...
TRANSCRIPT_LOGGER = logging.getLogger("transcript_logger")
...
def get_podcast_mp3_link(page_soup):
    ...
    if len(mp3_candidate_links) > 1:
        TRANSCRIPT_LOGGER.warning("More than 1 candidate mp3 URL found")
    else:
...
def create_manifest(
    audio_output_dir, transcript_output_dir, podcast_min_len=5, podcast_max_len=60
):
    transcript_records = []
